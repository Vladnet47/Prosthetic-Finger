...
CommandInterpreter::CommandInterpreter() {
	this->conversions = new CommandTypeMap[NUM_CONVERSIONS]{
...
int CommandInterpreter::parseCommands(Command*& commands, const char chars[], const int length) const {
	...
	while (index < count && end < length) {
		...
		// If end symbol is found, the symbols between the start and end are converted into a command object
			...
			const char* typeChars = arraySubset(chars, length, start + 1, start + 1 + COMMAND_LENGTH);

			// Parse command type
			enum CommandType type;
			bool successfullyParsed = this->tryParseType(type, typeChars, COMMAND_LENGTH);
			delete[] typeChars;
			...
			const char* dataChars = arraySubset(chars, length, start + 1 + COMMAND_LENGTH, end);

			// Parse int from data
			int nData;
			successfullyParsed = this->tryParseInt(nData, dataChars, dataLength);
...
bool CommandInterpreter::tryParseType(enum CommandType& type, const char chars[], const int length) const {
	...
	for (int i = 0; i < NUM_CONVERSIONS; ++i) {
		// Get required format
		const char* format = this->conversions->raw;
...
// undefined state.
bool CommandInterpreter::tryParseInt(int& result, const char chars[], const int length) const {
	if (chars == nullptr) {
		return false;
	}
	...
	for (int i = length - 1; i >= 0; --i) {
		...
		// If numeric, convert to integer
			...
			result += (c - '0') * this->pow(10, length - i - 1);
		}
...
int CommandInterpreter::pow(const int base, const int exponent) const {
	if (exponent <= 0) {
		return 1;
	}
...
char* CommandInterpreter::arraySubset(const char chars[], const int length, const int start, const int end) const {
	// Check if valid indeces/lengths
	const int subsetLength = end - start;
	if (chars == nullptr || length <= 0 || subsetLength > length) {
		return nullptr;
	}
